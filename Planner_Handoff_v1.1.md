...
## Version 1.1 additive update notice

**Version:** 1.1  
**Update method:** additive insertions only  
**Parent document:** `POS_Equipment_Planner_Handoff.md`, Version 1.0  
**Parent SHA-256:** `9368426ac3f63afad2030c26e2d71bd97d1300f54d3d60e62c6402a67527c18a`  
**Source HTML:** `Обладнання v 2.0.html`, 4,162,044 bytes as observed in the working environment  
**Purpose of Version 1.1:** close the domain, business-rule, migration, privacy, recovery, routing, search, quality and traceability gaps identified by the microscope audit.

### Preservation guarantee

Version 1.1 does not replace or paraphrase Version 1.0. It preserves the original text and introduces explicitly marked supplementary blocks in context. If a later statement appears to conflict with an earlier one, apply this precedence:

1. confirmed business decision recorded in the Decision Log;
2. Version 1.1 supplementary clarification;
3. Version 1.0 proposal;
4. an unconfirmed example.

### Statement-status labels

All future additions and decisions should use these labels:

- **[Observed]** directly present in a source artifact;
- **[Measured]** obtained through inspection or tooling;
- **[Decision]** explicitly accepted as the current direction;
- **[Proposed]** recommended but not yet confirmed;
- **[Open]** requires user or business validation;
- **[Deferred]** consciously postponed to a later phase;
- **[Constraint]** must not be violated without an explicit architecture decision.

### Language policy

- **Documentation language:** English is acceptable for technical portability between agents.
- **Product interface language:** Ukrainian.
- **Business terminology:** must be validated in Ukrainian with the actual representative workflow.
- **Code identifiers:** English unless a technical constraint requires otherwise.

### Change-control rule

Every later revision must:

1. keep the previous version available as a separate file;
2. add a change log;
3. preserve accepted decisions unless they are explicitly superseded;
4. record the reason for any superseded decision;
5. connect new requirements to acceptance criteria or tests.
...
# Supplement 7A. Canonical planning-unit decision gate

**Status:** [Open, blocking schema freeze]

The strategic product center remains the pharmacy, but the exact planning unit is not yet confirmed. Version 1.0 references `VisitPlan` extensively; this must not be interpreted as a final database decision.

## Candidate A: Pharmacy tasks only
...
Use when work is accumulated continuously for each pharmacy and later grouped by week.

## Candidate B: Visit-centered planning
...
Use when each physical visit is a meaningful historical and operational record.

## Candidate C: Weekly work batch
...
Use when the ordering cycle and weekly execution list are more important than individual visit history.

## Required discovery test

Before freezing storage schemas, obtain three realistic examples:

1. a note created during a visit;
2. a list used to prepare the next equipment order;
3. a case where unfinished work is carried to another week or visit.

Choose the model that represents all three with the fewest duplicated fields and the least manual re-entry.

## Canonical flow independent of the final model

```text
Select Pharmacy
→ Open or create the current work context
→ Select Equipment
→ Define Action, Quantity and Note
→ Save Task
...
- keep `EquipmentTask` independently identifiable;
- reference `pharmacyId` directly on the task;
- make `planId` optional in prototypes;
- do not create irreversible migrations around `VisitPlan`;
- isolate plan grouping behind repository/service APIs.
...
# Supplement 8A. Business Rules Specification

**Status:** mixed. Rules marked [Proposed] require validation before implementation freeze.

## 8A.1. Quantity

- **[Proposed]** Quantity is a positive whole number.
- **[Proposed]** Default quantity is `1`.
- **[Constraint]** UI and storage validation must use the same rule.
- **[Open]** Determine whether inspection, rebranding or POSM-only work may exist without an equipment-unit quantity.
- **[Open]** Determine whether replacement quantity represents new units required, old units removed, or both.

## 8A.2. Duplicate tasks

- **[Open]** Decide whether two active tasks may have the same `pharmacyId`, `equipmentId` and `action`.
- **[Proposed]** Do not silently merge tasks during editing.
- **[Proposed]** When adding a potential duplicate, show the existing task and offer:
  - increase quantity;
  - create a separate task;
  - cancel.
- **[Constraint]** Aggregation must not double-count due to hidden duplicate-merging behavior.

## 8A.3. Work-action semantics

### Install

New equipment is required at the pharmacy.

### Replace

**[Open]** Confirm whether this means:

- supply one new unit;
- remove one old unit;
- record both operations;
- or only record the required final state.

### Remove

Equipment is to be removed. **[Proposed]** It should not automatically contribute to new-equipment order quantity.

### Repair

**[Open]** Confirm whether spare parts or replacement equipment may be required.

### Rebrand / update POSM

**[Open]** Distinguish between:

- work performed on an equipment unit;
- consumption of a separate POSM material;
- a note-only instruction.

### Inspect

**[Open]** Confirm whether an inspection must always reference known equipment.

### Order

**[Open, blocking aggregation logic]** Choose exactly one meaning:

1. a user-created work action;
2. a task workflow status;
3. a derived export generated from install/replace requirements.

**[Proposed]** Treat ordering as a derived export or workflow status, not as a parallel action, unless the real process proves otherwise.
...
```text
draft → planned → ordered → completed
   └────────────→ cancelled
```
...
- whether `ordered` belongs to a task or an export/order batch;
- whether completed tasks can be reopened;
- whether cancelled tasks remain visible in history;
- whether editing a completed task creates a revision or mutates history;
- whether plans and tasks need separate statuses.

## 8A.5. Summary inclusion rules

Before aggregation is implemented, define a matrix for every action/status combination.

Initial proposal:

- include active install requirements;
- include active replacement requirements according to confirmed replacement semantics;
- exclude remove-only tasks from new-equipment quantity;
- exclude cancelled tasks;
- avoid counting completed tasks in a future order unless explicitly selected;
- make inspect, repair and rebrand inclusion configurable only after their material requirements are understood.

## 8A.6. Delete, archive and undo

- **[Proposed]** Use soft deletion or cancellation for saved operational tasks.
- **[Proposed]** Use short-lived undo for accidental removal from the current draft.
- **[Open]** Define data-retention duration.
- **[Constraint]** Historical exports must never change silently after later edits.

## 8A.7. POS, POSM and equipment terminology

- **Equipment:** reusable physical display, stand, tray, tower or fixture.
- **POSM:** point-of-sale material that may be attached to or used with equipment.
- **Task:** work to be performed in a pharmacy.

**[Open]** Confirm whether POSM must become a separate catalog entity. Do not force POSM consumables into the equipment quantity model without validation.

## 8A.8. Business-rule sign-off checklist

No full MVP schema or order aggregation should be frozen until the user confirms:
...
# Supplement 11A. Routes, Search, Draft State and Localization

## 11A.1. Proposed route map

**Status:** [Proposed]

```text
/                         → default application entry
/pharmacies               → pharmacy search and recent list
/pharmacies/:pharmacyId   → pharmacy workspace
/plans                    → plans/work batches
/plans/:planId            → plan details, if VisitPlan is confirmed
/catalog                  → standalone equipment reference
/summary                  → aggregate summary
/settings                 → backup, restore and application information
```

Task editing and equipment selection may use nested routes or accessible modal sheets. The chosen approach must preserve browser Back behavior.

## 11A.2. Navigation behavior

- Browser Back must close the topmost temporary layer before leaving the pharmacy context.
- Refreshing a valid nested route must restore the same screen.
- Invalid or missing IDs must lead to a recoverable not-found state.
- Unsaved edits must not disappear due to accidental Back navigation.
- Static hosting must be configured for SPA route fallback if real routes are used.

## 11A.3. Search specification

**Pharmacy search fields:** name, chain, address, city and external code.  
**Equipment search fields:** name, article, brand, category, classification and group.

Normalization should consider:

- Unicode normalization;
- Ukrainian case folding;
- apostrophe variants;
- repeated whitespace;
- punctuation in addresses and articles;
- exact article matches ranked above partial matches;
- starts-with matches ranked above contains matches;
- active records ranked above inactive records;
- recent and favorite items used only as secondary ranking signals.

**[Open]** Decide whether typo tolerance and transliteration are required. Do not add fuzzy matching before real queries demonstrate the need.

## 11A.4. Empty and zero-result states

A zero-result state must:

- repeat the query;
- offer filter reset;
- allow switching between pharmacy/equipment search scopes where relevant;
- explain whether inactive records are hidden;
- never appear as an empty blank page.

## 11A.5. Task draft behavior

**[Proposed]** Use explicit Save for a task plus temporary local draft preservation.

- preserve meaningful unsaved changes if a sheet or page closes accidentally;
- show a warning only when data would truly be lost;
- clear the temporary draft after successful save or explicit discard;
- avoid creating a permanent task record before the user has selected required fields.

## 11A.6. Locale and time policy

**[Proposed]**

```text
UI locale: uk-UA
Business week: ISO week, Monday through Sunday
Persist timestamps: ISO 8601 in UTC
Persist calendar-only business dates: YYYY-MM-DD
Display date: Ukrainian localized format
```

**[Open]** Confirm the company’s actual week and reporting-cycle rules before implementing weekly grouping.

## 11A.7. Device and browser discovery

Before PWA or export behavior is frozen, record:

- Android versus iOS usage;
- primary browsers and minimum versions;
- whether devices are company-managed;
- file-download restrictions;
- whether PWA installation is allowed;
- whether sharing through the native share sheet is useful;
- whether the application must support desktop-only Excel review.

## 11A.8. Performance budgets

Initial budgets are **[Proposed]** and must be measured on a representative mid-range phone:

- avoid rendering the full image-card catalog on initial load;
- thumbnail target normally below 50 KB where visual quality permits;
- full image target normally below 250 KB where visual quality permits;
- initial route should become interactive without waiting for full-size equipment images;
- search feedback should feel immediate for approximately 1,000 pharmacies and the current equipment catalog;
- IndexedDB operations in primary flows should not produce visible blocking;
- every budget must be tracked through build or test output rather than documentation alone.
...
# Supplement 12A. Source Data Import and Provenance Specification

## 12A.1. Source preservation

**[Constraint]** Keep the original HTML artifact immutable until an authoritative replacement data source is confirmed.
...
## 12A.2. Equipment extraction map

Each source product card should map to:
...
→ classification
→ group
→ image source or no-photo state
→ source search text, if retained only for reconciliation
```

## 12A.3. Stable ID strategy

**[Proposed]** Use an application-controlled stable ID, not mutable display text.

- An article may inform ID generation only if uniqueness and permanence are confirmed.
- Duplicate or missing articles must enter an exception report.
- Never silently overwrite one record with another during import.
- Preserve inactive records needed by saved historical tasks.

## 12A.4. Required import outputs

```text
public/data/equipment.json
public/data/equipment-version.json
public/assets/equipment/... image files
build-reports/image-manifest.json
build-reports/import-report.json
build-reports/validation-errors.json
build-reports/source-metadata.json
```

Reports are build artifacts and do not all need to be delivered to the browser.

## 12A.5. Image manifest
...
Exact duplicate images may be physically deduplicated only if every equipment record keeps the correct logical reference.

## 12A.6. Validation checks

At minimum:

- schema-valid records;
- unique stable IDs;
- article uniqueness report;
- required-name check;
- known-brand normalization report;
- valid image-path check;
- no-photo count;
- source-count reconciliation;
- orphan image detection;
- unresolved source-card report.

## 12A.7. Version-to-version comparison

Every catalog update should report:

- added equipment;
- removed or inactive equipment;
- changed metadata;
- changed images;
- changed articles;
- records that would break existing task references.

Saved tasks must resolve historical equipment even after a catalog update. If display metadata changes, decide whether a task shows current catalog text or a historical snapshot.

## 12A.8. Pharmacy import

The pharmacy import needs a parallel contract:
...
- territory subset rules;
- privacy approval status.

Do not publish a real pharmacy file until the privacy gate in Supplement 17A passes.
...
# Supplement 14A. Backup and Restore Contract

## 14A.1. Backup envelope
...
```json
{
  ...
  "referenceDataVersions": {
    "equipment": "version-id",
    "pharmacies": "version-id"
  },
  "data": {
    "plans": [],
    "tasks": [],
...
The exact fields are [Proposed], but version metadata is required.

## 14A.2. Restore modes

Before implementation, choose and label restore behavior:

- **Replace:** erase current local user data and load the backup.
- **Merge:** preserve current data and import non-conflicting records.

**[Proposed]** MVP should support Replace first because it is easier to make predictable. Merge should be added only with explicit conflict rules.

## 14A.3. Restore safety

The restore flow should:

1. validate file structure and schema version;
2. show a summary before writing;
3. create a temporary pre-restore backup where feasible;
4. perform migration in memory or a transaction;
5. commit atomically;
6. report failures without leaving a half-restored database;
7. offer rollback when possible.

## 14A.4. Reference-data mismatch

If the backup references older catalog versions:

- preserve task IDs and relationships;
- detect missing equipment or pharmacy references;
- show unresolved records clearly;
- never silently delete tasks;
- provide a reconciliation report.

## 14A.5. Round-trip invariant
...
→ import backup
→ compare plans, tasks and relationships
```

The restored business data must be equivalent to the exported data.

## 14A.6. Corrupted and unsupported files

- reject invalid JSON safely;
- reject unsupported future schema versions with a clear message;
- do not partially import a corrupted file;
- preserve the current database after a failed import;
- record no sensitive payload in console or telemetry logs by default.

## 14A.7. Backup sensitivity

**[Constraint]** Treat backup files as potentially sensitive because notes and pharmacy associations may reveal business activity.
...
# Supplement 17A. Privacy, Security and Deployment Gate

## 17A.1. Production data gate

**[Constraint]** Do not deploy a real pharmacy list to a publicly accessible static bundle until the data owner confirms that this is permitted.

A static frontend cannot hide data that it ships to the browser. Obscure URLs are not access control.

## 17A.2. Data classification questions

Before production:

- classify pharmacy reference data;
- classify representative notes;
- determine whether notes may include personal names, phone numbers or sensitive business observations;
- classify backup files;
- identify the organizational data owner;
- define who may access the application.

## 17A.3. Access-control decision

Choose one:

1. public application with approved non-sensitive reference data;
2. restricted static application behind an approved access layer;
3. authenticated application with a backend;
4. local/private prototype using sanitized data only.

Cloud hosting selection does not by itself solve access control.

## 17A.4. Local-device risk
...
- application reset;
- explicit “Delete all local data” action;
- backup before reset;
- device-level authentication expectations.

## 17A.5. Retention and deletion

**[Open]** Confirm:

- how long completed and cancelled work is retained;
- whether the user may permanently delete history;
- whether business records require retention;
- whether backups must follow the same retention rule.

## 17A.6. Analytics and telemetry

**[Open]** Decide whether analytics are allowed.

If analytics or error monitoring are introduced:

- collect the minimum data;
- do not send note text, pharmacy details or backup payloads;
- document the vendor and retention;
- support the required consent or organizational approval;
- keep the MVP functional without analytics.

Zero telemetry is a valid initial decision.

## 17A.7. Frontend security baseline

- use a Content Security Policy appropriate to the deployment;
- avoid rendering untrusted values through raw `innerHTML`;
- validate imported JSON/CSV/Excel content;
- pin and review dependencies;
- avoid secrets in frontend environment variables;
- use HTTPS-only production delivery;
- sanitize filenames and exported cell content;
- evaluate spreadsheet-formula injection for CSV/Excel exports;
- do not log sensitive user-created content.

## 17A.8. Security sign-off

Production release with real data requires documented decisions for:

- data classification;
- access model;
- local-storage acceptance;
- backup handling;
- retention/deletion;
- telemetry;
- incident response for a lost device or exposed backup.
...
# Supplement 18A. Do Not Reintroduce Checklist

The source artifact is a reference, not a template. The new build must not reintroduce these observed or inferred weaknesses:

## Interaction and accessibility

- clickable non-interactive cards with no keyboard equivalent;
- modal surfaces without dialog semantics;
- missing focus trap and focus restoration;
- dynamic feedback without `aria-live` or status messaging;
- quantity fields without accessible labels;
- search that relies only on placeholder text;
- hover-only discovery of essential actions;
- inconsistent focus-visible styling;
- touch targets substantially smaller than comfortable mobile targets;
- motion without reduced-motion support.

## Architecture and security

- a single monolithic HTML file containing content, CSS, JavaScript and images;
- Base64 embedding of the complete image catalog;
- duplicated hand-written product-card markup as the data source;
- direct `innerHTML` insertion of imported business data;
- user plans held only in memory;
- personal notes committed to static repository data;
- UI components directly coupled to IndexedDB;
- silent data migrations or destructive import behavior.

## Performance and maintainability

- rendering every large catalog card by default;
- repeated DOM scans for each search keystroke as data grows;
- unstable active-navigation behavior based on multiple competing intersections;
- external font dependence without an offline/fallback strategy;
- no validation report for generated data;
- no automated primary-flow, accessibility or backup tests.

# Supplement 18B. New Product Semantic States

The new application needs semantic states that were absent from the source design snapshot.

Create tokens and components for:
...
- cancelled;
- archived;
- unresolved reference after import.

Do not derive all states from the primary pink. Semantic colors must be accessible, distinguishable and accompanied by text or icon meaning.

# Supplement 18C. Browser and Device Support Matrix

Before release, maintain a tested matrix containing:
...
The matrix should reflect actual company devices rather than generic market assumptions.
...
# Supplement 22A. Version 1.1 Decision Status

## Confirmed by the user in this revision

- Version 1.0 content must be preserved rather than rewritten.
- Version 1.1 must be created through additive contextual supplements.
- The microscope-audit gaps must be incorporated before transfer to the next agent.

## Still open after Version 1.1

- canonical planning unit;
- exact ordering semantics;
- final action and status vocabularies;
- duplicate and quantity rules;
- privacy approval for real pharmacy data;
- exact export workbook contract;
- supported device/browser matrix;
- backup merge behavior;
- route-versus-modal task editing.

## Version 1.1 scope clarification

This revision strengthens the handoff and prevents silent assumptions. It intentionally does not pretend that unresolved business decisions have already been approved.
...
# Supplement 23A. Requirements Traceability

Every material requirement should receive a stable identifier.

Initial set:

```text
PRD-001  User can find a pharmacy by name, address, chain or code.
PRD-002  User can open a pharmacy workspace.
PRD-003  User can create an equipment task in a pharmacy context.
PRD-004  User can find equipment by name, article or brand.
PRD-005  User can set action, quantity and note.
PRD-006  User can review and edit saved work.
PRD-007  User can produce an aggregate summary.

BR-001   Quantity follows the confirmed quantity rule.
BR-002   Duplicate tasks follow the confirmed duplicate policy.
BR-003   Cancelled tasks follow the confirmed summary-exclusion rule.
BR-004   Replacement follows the confirmed supply/removal semantics.
BR-005   Ordering follows exactly one confirmed semantic model.

DATA-001 Equipment records use stable IDs.
DATA-002 Pharmacy records use stable IDs.
DATA-003 Imports produce validation and reconciliation reports.
DATA-004 Historical tasks survive reference-data updates.

OFF-001  Saved tasks remain available without network access.
OFF-002  Browser reload does not lose committed work.
OFF-003  Backup and restore satisfy the round-trip invariant.

EXP-001  Text export preserves pharmacy, equipment, action, quantity and note.
EXP-002  CSV export uses the approved column contract.
EXP-003  Aggregation can show contributing pharmacies.

ACC-001  Core flow is operable with keyboard and touch.
ACC-002  Dynamic result messages are announced accessibly.
ACC-003  Modal or sheet focus is managed correctly.

SEC-001  Real pharmacy data is not publicly deployed without approval.
SEC-002  Imported data is validated before persistence.
SEC-003  CSV/Excel export mitigates formula injection.
SEC-004  Sensitive note content is excluded from telemetry.
```

Tests, issues and pull requests should reference these IDs where applicable.

# Supplement 23B. Expanded Acceptance Scenarios

In addition to the minimal happy path, validate:

1. **Duplicate task:** adding the same equipment/action follows the chosen duplicate policy.
2. **Edit:** changing action, quantity or note updates summaries correctly.
3. **Cancel:** cancelled work is preserved in history and excluded according to business rules.
4. **Delete/undo:** accidental draft removal can be recovered according to the chosen UX.
5. **Offline creation:** a task can be created and reopened without network connectivity.
6. **Reload:** committed work survives browser reload.
7. **Missing image:** equipment remains usable without an image.
8. **Catalog update:** existing tasks remain resolvable after equipment data changes.
9. **Pharmacy deactivation:** old tasks remain available when a pharmacy is no longer active.
10. **CSV aggregation:** totals equal the included contributing tasks and exclude disallowed statuses/actions.
11. **Backup round trip:** export, clear and restore produces equivalent business data.
12. **Corrupted backup:** import fails safely without changing current data.
13. **Older schema backup:** supported migration succeeds or unsupported import explains why it cannot continue.
14. **Keyboard-only use:** the core flow is completed without a pointing device.
15. **Mobile navigation:** browser Back and bottom navigation preserve expected context.
16. **Unsaved task:** accidental close follows the confirmed draft-loss protection rule.
17. **Search normalization:** realistic Ukrainian names, addresses and articles are found as expected.
18. **Security:** CSV values beginning with spreadsheet formula characters are exported safely.

# Supplement 23C. Architecture Decision Records

For every major open decision, create an ADR containing:
...
- ADR-001 Planning unit: task, visit or weekly batch;
- ADR-002 Order semantics;
- ADR-003 Local persistence and repository boundary;
- ADR-004 Public versus restricted deployment;
- ADR-005 Backup replace versus merge;
- ADR-006 Route-based versus modal task editing;
- ADR-007 Reference-data versioning strategy.
...
# Appendix 0. Complete Source Design Reference Addendum

**Status:** [Observed/Measured reference]  
**Purpose:** preserve detailed source-design measurements.  
**Constraint:** this appendix does not require the new application to copy the old page composition.

## Source layout measurements

```text
Desktop layout: 220px sidebar + 1fr content
Page/header horizontal padding: 28px
Content padding: 26px 28px 90px
Product grid: repeat(auto-fill, minmax(230px, 1fr))
Grid gap: 16px
Section separation: 46px
Search maximum width: 480px
Source responsive breakpoint: 860px
Lightbox maximum width: 880px
Floating list panel width: 340px
```
...
- logo mark: `30 × 30px`;
- decorative glow: `320 × 320px`, top `-90px`, right `-70px`;
- logo-row gap: `10px`;
- logo-row bottom margin: `22px`;
- H1 maximum width: `760px`;
- lead maximum width: `640px`;
- statistics gap: `28px`.

### Search

- container padding: `13px 28px`;
- gap: `12px`;
- input padding: `10px 14px 10px 38px`;
- border: `1.5px solid` strong border;
- radius: `9px`;
- icon: `16px`, stroke `2px`, positioned `12px` from the left;
- focus ring: `0 0 0 3px #FFE3EC`.

### Sidebar navigation

- source width: `220px`;
- sticky top: `57px`;
- padding: `22px 14px 40px 28px`;
- item padding: `7px 10px`;
- item radius: `7px`;
- item gap: `8px`;
- item bottom interval: `2px`.
...
- bottom padding: `8px`;
- bottom margin: `18px`;
- divider: `2px solid primary pink`;
- section bottom margin: `46px`;
- scroll margin: `70px`.

### Product card

- background: white;
- resting border: `1px solid #EFE7EA`;
- radius: `12px`;
- resting shadow: none;
- hover lift: `-3px`;
- hover shadow: `0 10px 24px rgba(255,27,107,0.16)`;
- hover border: `#FFC4DC`;
- media ratio: `4:3`;
- image fit: `contain`;
- image padding: `8px`;
...
- article padding: `1px 6px`;
- article radius: `4px`;
- copy button: `22 × 22px`, radius `6px`;
- controls top margin/padding: `10px / 10px`;
- controls gap: `8px`;
- stepper segment widths: `24px / 32px / 24px`;
- stepper height: `26px`;
- add-button padding: `6px 10px`;
- add-button radius: `7px`.

### Lightbox

- backdrop padding: `24px`;
- backdrop: graphite at 72% opacity plus `3px` blur;
- dialog radius: `16px`;
- grid: `1.1fr 1fr`;
- max height: `88vh`;
- modal shadow: `0 30px 70px rgba(0,0,0,0.35)`;
- closed scale: `0.96`;
- close button: `34 × 34px`, top/right `12px`;
- media padding: `20px`;
- media minimum height: `280px`;
- content padding: `28px 26px`.

### Floating list

- FAB position: right/bottom `20px`;
- FAB padding: `12px 18px`;
- FAB radius: `30px`;
- FAB shadow: `0 8px 22px rgba(0,0,0,0.28)`;
- panel position: right `20px`, bottom `84px`;
- panel width: `340px`;
- panel max-height: `70vh`;
- panel radius: `14px`;
- panel shadow: `0 16px 40px rgba(0,0,0,0.28)`;
- closed panel offset: `8px` downward.

## Source gradients and effects
...
## Source icon inventory

- search: 16 px custom outline SVG, 2 px stroke, deep pink;
- copy: compact Unicode-style symbol, 12 px;
- quantity minus/plus: text symbols, 14 px;
- lightbox close: multiplication sign, 20 px;
- list close: multiplication sign, 15 px;
- item removal: multiplication sign, 16 px;
- expand hint: expand symbol plus text, 11.5 px;
- success: check mark;
- FAB: clipboard symbol/emoji plus text.

The new product should replace ad hoc Unicode icons with a consistent accessible icon set if this can be done without increasing visual complexity.

## Source z-index reference

```text
Sticky search: 50
Floating action and list panel: 150
Lightbox: 200
Lightbox close within local stacking context: 2
```

## Source typography usage reference

```text
H1: Poppins 700, clamp(22px, 3.4vw, 32px), -0.01em
H2: Poppins 700, 21px
Card title: Poppins 600, 15.5px
Modal title: Poppins 600, 21px
Lead: Inter 400, 14.5px
Logo: Poppins 800, 22px, -0.02em
Statistics number: Poppins 700, 23px
Navigation: Inter 400, 13.5px
Metadata: Inter 400, 12.5px
Data labels and counts: monospace, 11px–12.5px
Primary button: Inter 600, 12px
FAB: Inter 600, 13.5px
```

## Source mobile adaptations at 860px and below

- desktop grid becomes one column;
- sidebar becomes static, wrapped navigation;
- sidebar padding becomes `16px 20px`;
- lightbox becomes one column;
- lightbox max-height becomes `92vh`;
- media minimum height becomes `220px`;
- list panel width becomes `calc(100vw - 24px)`;
- panel right offset becomes `12px` and bottom `78px`;
- FAB right/bottom becomes `12px`.

These measurements are preserved for design archaeology. The new mobile-first application is expected to use a different information architecture.
